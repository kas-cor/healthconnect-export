...
- **Переключение темы** — светлая/тёмная/как в системе с анимированным переходом
- **Переключение языка** — английский / русский, сохраняется между сессиями
...
```bash
# Запустить все модульные тесты (323 теста)
...
| `DataModelsSerializationTest` | 33 | Обратная сериализация: DailyHealthRecord, ExportConfig, enum'ы, SpeedData, sourceDisplayName |
...
| `DailyExportWorkerTest` | 25 | `doWork()` (успех/уже экспортировано/пусто/исключения) + `schedule()` (ежедневно/еженедельно/вручную/отмена) |
...
| `ExportDataUseCaseTest` | 15 | Рабочий процесс экспорта: разрешения, проверка Health Connect, прогресс, webhook, Drive |
| `ExportViewModelTest` | 13 | Состояния ViewModel: загрузка, экспорт, ошибка, разрешения, расписание, Drive, тест webhook |
| `LocaleManagerTest` | 12 | localeDisplayName (все ветки), saveLocale/getSavedLocale |
| `DateRangeCardTest` | 8 | Compose UI тесты: пресеты, произвольные даты, взаимодействие с календарём |
...
- **Переключение языка**: через иконку языка в верхней панели приложения
- **Покрытие**: Все **136 строк** переведены в `values-ru/strings.xml`
