...
- **Диагностика доставки** — во вкладке «План» видно время и HTTP-код последней успешной отправки, а также предупреждение, если данные не доставлялись 6+ часов
- **Догоняющая отправка при запуске** — если последняя успешная отправка старше 6 часов, пропущенные дни уходят сразу при открытии приложения (окно ограничено 7 днями); кнопка «Отправить сейчас» запускает то же вручную
- **Надёжность фона** — периодические задачи перерегистрируются после перезагрузки и обновления приложения, во вкладке «План» есть отключение оптимизации батареи в один тап для ROM'ов, которые «убивают» фоновые задачи
